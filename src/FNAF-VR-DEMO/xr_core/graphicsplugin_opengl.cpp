...
#include "VR/Scene.h"
#include "VR/XrCallbacks.h"

#define GLFW_EXPOSE_NATIVE_WIN32
#define GLFW_EXPOSE_NATIVE_WGL
#define GLFW_NATIVE_INCLUDE_NONE
#include <GLFW/glfw3native.h>
#include <xr_common/gfxwrapper_opengl.h>
...
//        }
        ...
        m_graphicsBinding.hDC = GetDC(glfwGetWin32Window(GlfwWindow::window));
        m_graphicsBinding.hGLRC = glfwGetWGLContext(GlfwWindow::window);
    ...
    void InitializeResources() {
        ...
        XrCallbacks::SetSwapChainFbo(m_swapchainFramebuffer);
    ...
    void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
        ...
        Scene::DisplayVr(glm::make_mat4(proj.m), glm::make_mat4(view.m));

        glBindFramebuffer(GL_FRAMEBUFFER, 0);

        bool mirror_display = true;

        if (mirror_display == false)
        {
            Scene::bClearDefaultFb = true;
        }
        //Mirror to glfw window
        if(abs(layerView.fov.angleLeft) < abs(layerView.fov.angleRight)) //HACK TO TELL LEFT EYE FROM RIGHT
        {
            if (mirror_display == true)
            {
                int src_w = layerView.subImage.imageRect.extent.width;
                int src_h = layerView.subImage.imageRect.extent.height;

                int dst_w, dst_h;
                glfwGetWindowSize(GlfwWindow::window, &dst_w, &dst_h);

                glDrawBuffer(GL_BACK);
                glNamedFramebufferReadBuffer(m_swapchainFramebuffer, GL_COLOR_ATTACHMENT0);
                glNamedFramebufferDrawBuffer(0, GL_BACK);
                glClear(GL_DEPTH_BUFFER_BIT);
                glBlitNamedFramebuffer(m_swapchainFramebuffer, 0,
                    0, 0, src_w, src_h,
                    0, 0, dst_w, dst_h,
                    GL_COLOR_BUFFER_BIT, GL_LINEAR);
            }
            Scene::Display(GlfwWindow::window); //swaps buffers
        }
