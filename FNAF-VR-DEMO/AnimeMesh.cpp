...
void AnimeMesh::VertexBoneData::AddBoneData(unsigned int BoneID, float Weight)
{
...
void AnimeMesh::CalcMeshBoundingBox(const aiMesh* mesh, aiVector3D* min, aiVector3D* max)
{
...
void AnimeMesh::CalcNodeBoundingBox(const aiNode* nd, aiVector3D* min, aiVector3D* max)
{
...
void AnimeMesh::CalcBoundingBox()
{
...
bool AnimeMesh::LoadMesh(const std::string& filename)
{
    ...
    // AssetLoader loader;
    mScene = mImporter.ReadFile(filename.c_str(), aiProcessPreset_TargetRealtime_Quality | aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs);

    if (mScene) {
        m_GlobalInverseTransform = mScene->mRootNode->mTransformation;
        m_GlobalInverseTransform.Inverse();
        ret = InitFromScene(mScene, filename.c_str());
        CalcBoundingBox();
    } else {
...
void AnimeMesh::Update(float deltaSeconds)
{
...
void AnimeMesh::InitMesh(unsigned int MeshIndex,
    const aiMesh* pMesh,
    std::vector<aiVector3D>& Positions,
    std::vector<aiVector3D>& Normals,
    std::vector<aiVector2D>& TexCoords,
...
void AnimeMesh::LoadBones(unsigned int MeshIndex, const aiMesh* pMesh, std::vector<VertexBoneData>& Bones)
{
...
bool AnimeMesh::InitMaterials(const aiScene* pScene, const std::string& Filename)
{
...
unsigned int AnimeMesh::FindRotation(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
unsigned int AnimeMesh::FindScaling(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void AnimeMesh::CalcInterpolatedPosition(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void AnimeMesh::CalcInterpolatedRotation(aiQuaternion& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void AnimeMesh::CalcInterpolatedScaling(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
...
void AnimeMesh::ReadNodeHierarchy(float AnimationTime, const aiNode* pNode, const aiMatrix4x4& ParentTransform)
{
...
void AnimeMesh::BoneTransform(float TimeInSeconds, std::vector<aiMatrix4x4>& Transforms)
{
...
const aiNodeAnim* AnimeMesh::FindNodeAnim(const aiAnimation* pAnimation, const std::string& NodeName)
{
